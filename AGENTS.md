...
A Tauri 2 menu-bar / system-tray app monitoring Codex and OpenAI Codex usage limits. Rust core + React/TypeScript UI. Reads the official CLIs' stored OAuth credentials **read-only** and calls each provider's own usage API — no servers, no telemetry.
...
cargo run --example live_fetch [Codex|codex]   # live fetch w/ your real creds, prints normalized snapshot
```
...
- **macOS signing env:** do not export empty Apple signing/notarization env vars in release workflows. Tauri interprets present-but-empty `APPLE_CERTIFICATE`/related vars as a signing attempt and fails with `SecKeychainItemImport`; leave them unset for unsigned DMG builds.
